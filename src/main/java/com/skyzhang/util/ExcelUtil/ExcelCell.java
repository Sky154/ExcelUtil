package com.skyzhang.util.ExcelUtil;

import java.lang.annotation.ElementType;
import java.lang.annotation.Retention;
import java.lang.annotation.RetentionPolicy;
import java.lang.annotation.Target;


/**
 * The <code>ExcelCell</code><br>
 * 数值型的栏位只能使用Double
 * @see {@link com.skyzhang.util.ExcelUtil.ExcelUtil#exportExcel}
 * @author sargeras.wang
 * @version 1.0, Created at 2013年09月14日
 * @version 1.1, Updated at 2013年10月20日 ,添加了专用于验证的子注解:Valid
 * @version 1.3, Updated at 2020年04月24日 ,添加了用户是否忽略导入导出
 */
@Retention(RetentionPolicy.RUNTIME)
@Target(ElementType.FIELD)
public @interface ExcelCell {
    /**
     * 顺序 default 100
     * 
     * @return index
     */
    int index();

    /**
     * 当值为null时要显示的值 default StringUtils.EMPTY
     * 
     * @return defaultValue
     */
    String defaultValue() default "";
    
    /**
     * 是否直接导入/出时忽略（默认不忽略）
     * @return boolean
     */
    boolean ignore() default false;
    
    /**
     * 是否导入时忽略（默认不忽略）
     * @return boolean
     */
    boolean ignoreImport() default false;
    
    /**
     * 是否导入时忽略（默认不忽略）
     * @return boolean
     */
    boolean ignoreExport() default false;

    /**
     * 用于验证
     * 
     * @return valid
     */
    Valid valid() default @Valid();

    @Retention(RetentionPolicy.RUNTIME)
    @Target(ElementType.FIELD)
    @interface Valid {
        /**
         * 必须与in中String相符,目前仅支持String类型
         * 
         * @return e.g. {"key","value"}
         */
        String[] in() default {};

        /**
         * 是否允许为空,用于验证数据 default true
         * 
         * @return allowNull
         */
        boolean allowNull() default true;

        /**
         * Apply a "greater than" constraint to the named property
         * <br/>大于
         * @return gt
         */
        double gt() default Double.NaN;

        /**
         * Apply a "less than" constraint to the named property
         * <br/>小于
         * @return lt
         */
        double lt() default Double.NaN;

        /**
         * Apply a "greater than or equal" constraint to the named property
         * <br/>大于等于
         * @return ge
         */
        double ge() default Double.NaN;

        /**
         * Apply a "less than or equal" constraint to the named property
         * <br/>小于等于
         * @return le
         */
        double le() default Double.NaN;
    }
}
